...
st.write('---')
st.subheader('Humidade')
st.line_chart(data=leituras, x='Datetime', y='HumOut', color='#00FFFF')
st.write(f'Humidade mínima: {leituras.HumOut.min()} %')
st.write(f'Humidade máxima: {leituras.HumOut.max()} %')
st.write('---')
st.subheader('Radiação Solar')
st.line_chart(data=leituras, x='Datetime', y='SolarRad', color='#00FA9A')
